...
**Model choice (validated live 2026-08-04):** the free models are not interchangeable. `ling-3.0-flash-free` is the primary (`AI_MODEL_MENU_IMPORT`, set as a Vercel production env var) — it is fast (~13s/chunk) and the only free model observed completing imports end-to-end. `deepseek-v4-flash-free` is the fallback; when it was tried as primary (2026-08-04/05) it failed 2/2 imports because it is slow (2-4 min per 6k-char chunk) and returns truncated/non-JSON output on large chunks, and `mimo-v2.5-free` was dropped for returning non-JSON output even on small prompts. `withFallback` retries each model twice before failing. Paid models (e.g. `deepseek-v4-flash`) return 401 `Insufficient balance` on this account, so the free roster is the only option for now. If a smarter model is needed, the levers are a paid model on the account, or smaller `CHUNK_SIZE` to give deepseek a chance on shorter inputs.
