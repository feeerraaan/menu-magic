...
**Production import backend (Vercel, client-driven steps):** long menu imports run through `api/ai-import-start.ts` (creates the `ai_jobs` row with the raw source and returns) plus `api/ai-import-step.ts` (executes exactly one step: text prep, one chunk extraction, or one language translation) and are selected by the frontend in prod / via `VITE_AI_IMPORT_BACKEND`. The frontend (`useAiImport`) drives the loop, calling the step endpoint until the job completes. Because the client (not the function itself) chains the steps, each invocation is a single short LLM call — **a menu of any length can be imported with no platform duration limit**, and there is no self-invocation so Vercel's **508 INFINITE_LOOP_DETECTED** protection can never fire (an earlier design chained steps by POSTing back to `/api/ai-import-worker`, which Vercel killed mid-run; a single synchronous `waitUntil` call worked but was capped at the Hobby 300s function limit). State lives in `ai_jobs.input` between steps (retry-safe: each step reads fresh persisted state). Chunk size is 6000 chars (overlap 600). Per-LLM-call timeouts are generous (90s inactivity / 240s hard) and each step still guards with a ~280s deadline so a stalled model call fails cleanly instead of hitting the 300s platform kill.

**Model choice + schema-repair (validated live 2026-08-05):** `deepseek-v4-flash-free` is the primary (`AI_MODEL_MENU_IMPORT`, set as a Vercel production env var) with `mimo-v2.5-free` as fallback. On large 6k-char chunks deepseek is slow (~200s+) and often returns invalid/truncated JSON, so `callStructured` implements **schema-repair** (the same pattern as mindmap's `withSchemaRepair`): when the output fails JSON parse / Zod validation, the model is re-called with its own raw output plus the validation error and told to fix it — up to `REPAIR_MAX_RETRIES` (2) corrective turns before giving up on that model. Verified live: a 6008-char chunk produced invalid JSON on the first call (214s) and a complete valid 138-item menu on the repair turn (33s). Each model is tried once (`ATTEMPTS_PER_MODEL=1`), with the repair loop handling same-model retries; `ling-3.0-flash-free` was removed because it parses acceptably but the user found its output quality worse, and `mimo` remains only as the last-resort fallback. Paid models (e.g. `deepseek-v4-flash`) return 401 `Insufficient balance` on this account, so the free roster is the only option for now; mindmap uses the paid `deepseek-v4-flash` via the `https://opencode.ai/zen/go/v1` gateway if that ever becomes available here.
