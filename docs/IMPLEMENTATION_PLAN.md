...
**Model choice (validated live 2026-08-04):** the free models are not interchangeable. `deepseek-v4-flash-free` is the primary (`AI_MODEL_MENU_IMPORT`, set as a Vercel production env var) — now viable because the step-based driver splits work into short per-chunk calls, so its slow large-input latency (up to ~95s on a 5.3k-char chunk) fits comfortably under the per-step deadline. `ling-3.0-flash-free` is the fallback (fast and reliable when deepseek fails; `mimo-v2.5-free` was dropped because it returns non-JSON / structurally-invalid output even on small prompts). `withFallback` retries each model twice before failing. Paid models (e.g. `deepseek-v4-flash`) return 401 `Insufficient balance` on this account, so the free roster is the only option for now.
