...
**Production import backend (Vercel, single synchronous call):** long menu imports run through `api/ai-import-start.ts` (Node, max 300s on the Hobby plan), selected by the frontend in prod / via `VITE_AI_IMPORT_BACKEND`. It does the whole pipeline — text extraction → chunked extraction calls → per-language batch translation → progress in `ai_jobs` — inside **one invocation's `waitUntil` background task**, then writes the completed `output` and charges `ai_usage`. Deliberately **no self-enqueueing worker**: an earlier design chained steps by POSTing back to `/api/ai-import-worker`, which Vercel kills mid-run with **508 INFINITE_LOOP_DETECTED** (a function calling itself; jobs stuck at partial progress). Chunk size is 6000 chars (overlap 600). Per-LLM-call timeouts are generous (90s inactivity / 240s hard) and `TOTAL_BUDGET_MS` (290s) sits just under the Vercel Hobby 300s function cap so a genuinely huge menu fails cleanly with a user-facing message instead of being silently killed and stuck in `processing`; the durable unlimited-duration alternative is Vercel Workflows (or a client-driven step loop) if a menu ever outgrows the single-call budget.

**Model choice (validated live 2026-08-04):** the free models are not interchangeable. `deepseek-v4-flash-free` is slow (~95s on a 5.3k-char chunk, blowing the 120s hard timeout) and `mimo-v2.5-free` returns non-JSON / structurally-invalid output even on small prompts. `ling-3.0-flash-free` is the reliable one (valid JSON for a full 28-item menu in ~13s) and is the primary model (`AI_MODEL_MENU_IMPORT`, set as a Vercel production env var); `deepseek-v4-flash-free` remains as the secondary fallback and `mimo` was dropped. `withFallback` now retries each model twice before failing. Paid models (e.g. `deepseek-v4-flash`) return 401 `Insufficient balance` on this account, so the free roster is the only option for now.
