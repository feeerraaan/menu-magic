...
|---|---|---|---|---|
| 2026-08-06 | i18n sweep + screenshots re-captured | `src/pages/dashboard/Admin.tsx`, `src/components/dashboard/admin/RestaurantDetail.tsx`, `src/pages/dashboard/MenuEditor.tsx`, `src/components/dashboard/DashboardSidebar.tsx`, `src/components/subscription/LimitIndicator.tsx`, `src/lib/subscription-limits.ts`, `src/locales/{es,en,ca}.json`, `src/pages/dashboard/AiCopilot.tsx`, `src/components/public/CustomerAssistantWidget.tsx`, `src/index.css`, `packages/ai/prompts/{copilot,customerAssistant}.ts`, `docs/screenshots/*.png` | No hardcoded user-facing text remains: backoffice (Admin + RestaurantDetail) fully i18n'd (78 new `admin.*` keys, en/es/ca in sync), editor limit labels via `limits.*`, editor dialogs and sidebar credit label via existing keys. Copilot chat now renders markdown (react-markdown) so bold/lists display; Copilot + Customer Assistant prompts forbid the em dash. Verified live: backoffice/editor in English, copilot executed action in English with no em-dash. All 12 screenshots re-taken; demo data restored after. | Task 2: video script; Task 14: trailer. Task 13 (whitepaper) done. |
