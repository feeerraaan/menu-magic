...
|---|---|---|---|---|
| 2026-08-05 | Task 9 — Landing improvements | `pages/Index.tsx`, `components/landing/ProblemSection.tsx` (new), `components/landing/SolutionSection.tsx` (new), `hooks/useTranslation.ts` (added `tRaw` for JSON arrays), `locales/{en,es,ca}.json` | Pushed to prod (commit `367be76`). Hero rewritten to transformation copy + AI badge; new **Problem** section (6h/week, never translated, zero visibility) and **How AI solves it** (Upload→Build→Translate→Optimize) sections; Features rewritten as benefits. i18n in sync (595 keys). tsc/build green, no new lint. | Before/After comparison is Task 10 (next). |
